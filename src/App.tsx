...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    switch (appState) {
      ...
      // Fix: Combine MASKING and GENERATING states to resolve type error.
      // By using a fallthrough, the type of `appState` is not narrowed to a single value,
      // making the `appState === AppState.GENERATING` comparison valid.
      // This also improves the UI by showing the loader in-place without losing context.
      ...
      case AppState.COMPARING:
        return (
          <div className="w-full max-w-5xl mx-auto flex flex-col gap-8 items-center">
            {originalImage && editedImages && <ImageComparator original={originalImage} editedImages={editedImages} />}
            <button
              onClick={handleReset}
